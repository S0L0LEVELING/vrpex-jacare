		...
		if vRP.getInventoryItemAmount(user_id,"lockpick") >= 1 then
			...
			if vehicle then
				...
				if not plateId then
					...
					TriggerClientEvent("Notify",source,"amarelo","Este veículo é protegido pela seguradora.",1000)
				end
			end
		end
		return false
	...
	if user_id then
		if timeList > 0 and userList[user_id] then
			...
			if vehListNames ~= "" then
				TriggerClientEvent("Notify",source,"amarelo",vehListNames.." a lista é atualizada em <b>"..parseInt(timeList).." minutos</b>, cada veículo entregue o mesmo é removido da lista.",25000)
			else
