	...
	while true do
		...
		if (IsPedRunning(ped) or IsPedSprinting(ped)) and tackleSystem <= 0 and not IsPedSwimming(ped) then
			...
			if IsControlJustPressed(1,38) then
				...
				if userStatus then
					TriggerServerEvent("vrp_tackle:Update",GetPlayerServerId(userStatus))
					tackleSystem = 3
...
AddEventHandler("vrp_tackle:Player",function()
...
function nearestPlayers()
	...
	local coords = GetOffsetFromEntityInWorldCoords(ped,0.0,1.2,1.0)
