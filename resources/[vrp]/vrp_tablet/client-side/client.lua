...
RegisterCommand("tablet",function(source,args)
	...
	if not IsPauseMenuActive() and GetEntityHealth(ped) > 101 and not IsEntityInWater(ped) then
...
local benCoords = { -57.28,-1095.3,26.43 }
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUESTDRIVE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("requestDrive",function(data, cb)
	...
	if distance <= 3 then
		...
		if driveIn then
			...
			testDriveEntity = vehCreate(data["name"],"TEST",-46.68,-1077.66,26.72,76.28)
			Citizen.Wait(3000)
