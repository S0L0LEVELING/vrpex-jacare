...
$(document).on("click","#pixSubmit",function(e){
	...
	if (value > 0 && target > 0){
		$.post("http://vrp_tablet/inputPix",JSON.stringify({ value: value, target: target }));
	}
	$("#pixValue").val('')
	$("#pixTarget").val('')
});
...
const benefactor = (mode) => {
	...
	$.post(`http://vrp_tablet/request`+ mode,JSON.stringify({}),(data) => {
		let i = 0;
...
$(document).on("click","#benefactorBuy",function(e){
	$.post("http://vrp_tablet/buyDealer",JSON.stringify({name: e["target"]["dataset"]["name"]}));
	benefactor("Carros");
});
/* ----------BENEFACTORRENTAL---------- */
...
$(document).on("click","#benefactorSell",function(e){
	$.post("http://vrp_tablet/sellDealer",JSON.stringify({name: e["target"]["dataset"]["name"]}));
	$.post("http://vrp_tablet/closeSystem");
	benefactor('Carros');
});
/* ----------BENEFACTORTAX---------- */
$(document).on("click","#benefactorTax",function(e){
	$.post("http://vrp_tablet/requestTax",JSON.stringify({ name: e["target"]["dataset"]["name"] }));
	benefactor('Carros');
});
/* ----------BENEFACTORDRIVE---------- */
$(document).on("click","#benefactorDrive",function(e){
	$.post("http://vrp_tablet/requestDrive",JSON.stringify({ name: e["target"]["dataset"]["name"] }));
	benefactor('Carros');
	$.post("http://vrp_tablet/closeSystem");
});
/* ----------FORMAT---------- */
