...
function emP.checkPermission()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		return vRP.hasPermission(user_id,"Bennys")
	end
end
	...
	if user_id then
		...
		if vehicle then
			...
			if plateUserId then
				...
				custom.color = { veh.color[1],veh.color[2] }
				custom.extracolor = { veh.extracolor[1],veh.extracolor[2] }
				custom.neon = veh.neon
				custom.neoncolor = { veh.neoncolor[1], veh.neoncolor[2], veh.neoncolor[3] }
				custom.xenoncolor = veh.xenoncolor
				custom.smokecolor = { veh.smokecolor[1], veh.smokecolor[2], veh.smokecolor[3] }
